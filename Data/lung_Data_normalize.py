...
path_annotation = "original/lung/annotation_global.csv"
path_meth = "original/lung/Matrix_meth.csv"
max_features = 5000
...
data.to_csv("./data/lung/preprocessed_annotation_global.csv", index=False)

matrixname = "meth"
data = pd.read_csv("./original/lung/Matrix_" + matrixname + ".csv", sep='\t').transpose()
...
data2 = data2.sort_values(by="Composite Element REF").drop(columns="Composite Element REF").dropna(axis=1)
data2.to_csv("./data/lung/Matrix_meth.csv", index=False, header=False)
...
data.to_csv("./data/lung/preprocessed_Matrix_" + matrixname + ".csv", index=False, header=False)
...
data = pd.DataFrame(data.values[1:], columns=headers)
index = data["Composite Element REF"]
data_onlyvalues = data.drop(columns=["Composite Element REF"]).dropna(axis=1)
data2 = pd.DataFrame(stats.zscore(data_onlyvalues.values.tolist()))
data2["Composite Element REF"] = index
data2 = data2[~data2["Composite Element REF"].isin(cases_removed["case_id"])]
data2 = data2.sort_values(by="Composite Element REF").drop(columns="Composite Element REF")
data2.to_csv("./data/lung/Matrix_miRNA.csv", index=False, header=False)
# data=data[data_onlyvalues.std().sort_values(ascending=False).head(max_features).index]
data = pd.DataFrame(
    stats.zscore(data[data_onlyvalues.std().sort_values(ascending=False).head(max_features).index].values.tolist()))
data["Composite Element REF"] = index
...
data2 = data2[~data2["Composite Element REF"].isin(cases_removed["case_id"])]
data2 = data2.sort_values(by="Composite Element REF").drop(columns="Composite Element REF")
data2.to_csv("./data/lung/Matrix_mRNA.csv", index=False, header=False)
# data=data[data_onlyvalues.std().sort_values(ascending=False).head(max_features).index]
data = pd.DataFrame(
    stats.zscore(data[data_onlyvalues.std().sort_values(ascending=False).head(max_features).index].values.tolist()))
data["Composite Element REF"] = index
