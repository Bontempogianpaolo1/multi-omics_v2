...
class MLP(nn.Module):
    def __init__(self, input_size=7, hidden_size=20, output_size=5):
        ...
        self.fc1.weight=torch.nn.Parameter(torch.rand(self.fc1.weight.shape,generator=torch.manual_seed(1)))
        self.fc1.bias=torch.nn.Parameter(torch.rand(self.fc1.bias.shape,generator=torch.manual_seed(1)))
        self.out = nn.Linear(hidden_size, output_size)
        self.out.weight = torch.nn.Parameter(torch.rand(self.out.weight.shape,generator=torch.manual_seed(1)))
    ...
    def loss(self, y_pred, target):
        ...
        return lr*criterion(y_pred, target.long())
